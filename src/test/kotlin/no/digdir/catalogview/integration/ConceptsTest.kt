...
import no.digdir.catalogview.utils.apiAuthorizedRequest
import no.digdir.catalogview.utils.jwk.JwtToken
...
class ConceptsTest : ApiTestContext() {
    ...
    internal inner class GetConceptById {
        ...
        fun `Responds with Not Found when not found in concept catalog`() {
            val response =
                apiAuthorizedRequest("/catalogs/111222333/concepts/321", port, null, JwtToken("111222333").toString(), HttpMethod.GET)
        ...
        fun `Responds with Not Found when not in relevant catalog`() {
            val response =
                apiAuthorizedRequest("/catalogs/333222111/concepts/123", port, null, JwtToken("333222111").toString(), HttpMethod.GET)
            Assertions.assertEquals(HttpStatus.NOT_FOUND.value(), response["status"])
        }
    ...
    internal inner class GetConcepts {
        ...
        fun `Param changedAfter filters correctly`() {
            val before =
                apiAuthorizedRequest(
                    "$path?changedAfter=2022-01-01T09:00:00.000-01:00",
                    port,
                    null,
                    JwtToken("111222333").toString(),
                    HttpMethod.GET,
                )
            ...
            val after =
                apiAuthorizedRequest(
                    "$path?changedAfter=2022-01-01T15:00:00.000-01:00",
                    port,
                    null,
                    JwtToken("111222333").toString(),
                    HttpMethod.GET,
        ...
        fun `Param domainCodes and changedAfter filters correctly`() {
            val response =
                apiAuthorizedRequest(
                    "$path?domainCodes=1,555&changedAfter=2022-01-01T09:00:00.000-01:00",
                    port,
                    null,
                    JwtToken("111222333").toString(),
                    HttpMethod.GET,
            ...
            val after =
                apiAuthorizedRequest(
                    "$path?domainCodes=1,555&changedAfter=2022-01-01T15:00:00.000-01:00",
                    port,
                    null,
                    JwtToken("111222333").toString(),
                    HttpMethod.GET,
            ...
            val otherDomain =
                apiAuthorizedRequest(
                    "$path?domainCodes=555&changedAfter=2022-01-01T09:00:00.000-01:00",
                    port,
                    null,
                    JwtToken("111222333").toString(),
                    HttpMethod.GET,
