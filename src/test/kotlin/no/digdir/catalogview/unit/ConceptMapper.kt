package no.digdir.catalogview.unit
...
import no.digdir.catalogview.model.Endringslogelement
import no.digdir.catalogview.model.ForholdTilKildeEnum
import no.digdir.catalogview.model.InterntFelt
import no.digdir.catalogview.model.Kildebeskrivelse
import no.digdir.catalogview.model.Kontaktpunkt
...
import no.digdir.catalogview.model.Term
...
import no.digdir.catalogview.service.toExternalDTO
import no.digdir.catalogview.utils.BOOLEAN_FIELD
import no.digdir.catalogview.utils.CODE_1
import no.digdir.catalogview.utils.CODE_2
import no.digdir.catalogview.utils.CODE_3
import no.digdir.catalogview.utils.CODE_LISTS
import no.digdir.catalogview.utils.CODE_LIST_0
import no.digdir.catalogview.utils.CODE_LIST_1
import no.digdir.catalogview.utils.CODE_LIST_FIELD
import no.digdir.catalogview.utils.EMPTY_ADMIN_DATA
import no.digdir.catalogview.utils.EMPTY_CONCEPT
import no.digdir.catalogview.utils.EMPTY_INTERNAL_CONCEPT
import no.digdir.catalogview.utils.INTERNAL_FIELDS
import no.digdir.catalogview.utils.TEXT_LONG_FIELD
import no.digdir.catalogview.utils.TEXT_SHORT_FIELD
import no.digdir.catalogview.utils.USER_LIST_FIELD
...
class ConceptMapper {
    ...
    fun `Map assigned user`() {
        val adminUser =
            AdminUser(id = "user-id", catalogId = "123456789", name = "John Doe", email = "epost@asdf.no", telephoneNumber = "11122233")
        val user = User(name = "John Doe", email = "epost@asdf.no")
        val expected = EMPTY_CONCEPT.copy(assignedUser = user)

        val result =
            EMPTY_INTERNAL_CONCEPT
                .copy(assignedUser = adminUser.id)
                .toExternalDTO(
    ...
    fun `Map domain codes`() {
        ...
        val result =
            EMPTY_INTERNAL_CONCEPT
                .copy(fagområdeKoder = listOf("0", "1"))
                .toExternalDTO(
    ...
    fun `Map boolean internal fields`() {
        ...
        val resultTrue =
            EMPTY_INTERNAL_CONCEPT
                .copy(interneFelt = mapOf(BOOLEAN_FIELD.id to InterntFelt("true")))
                .toExternalDTO(EMPTY_ADMIN_DATA.copy(internalFields = INTERNAL_FIELDS), "http://localhost:5555")
        val resultFalse =
            EMPTY_INTERNAL_CONCEPT
                .copy(interneFelt = mapOf(BOOLEAN_FIELD.id to InterntFelt("false")))
                .toExternalDTO(EMPTY_ADMIN_DATA.copy(internalFields = INTERNAL_FIELDS), "http://localhost:5555")
        val resultInvalid =
            EMPTY_INTERNAL_CONCEPT
                .copy(interneFelt = mapOf(BOOLEAN_FIELD.id to InterntFelt("invalid")))
                .toExternalDTO(EMPTY_ADMIN_DATA.copy(internalFields = INTERNAL_FIELDS), "http://localhost:5555")
    ...
    fun `Map short text internal fields`() {
        ...
        val result =
            EMPTY_INTERNAL_CONCEPT
                .copy(interneFelt = mapOf(TEXT_SHORT_FIELD.id to InterntFelt("short string value")))
                .toExternalDTO(EMPTY_ADMIN_DATA.copy(internalFields = INTERNAL_FIELDS), "http://localhost:5555")
    ...
    fun `Map long text internal fields`() {
        ...
        val result =
            EMPTY_INTERNAL_CONCEPT
                .copy(interneFelt = mapOf(TEXT_LONG_FIELD.id to InterntFelt("long string value")))
                .toExternalDTO(EMPTY_ADMIN_DATA.copy(internalFields = INTERNAL_FIELDS), "http://localhost:5555")
    ...
    fun `Map code list internal fields`() {
        ...
        val result =
            EMPTY_INTERNAL_CONCEPT
                .copy(interneFelt = mapOf(CODE_LIST_FIELD.id to InterntFelt("${CODE_1.id}")))
                .toExternalDTO(
        ...
        val resultInvalid =
            EMPTY_INTERNAL_CONCEPT
                .copy(interneFelt = mapOf(CODE_LIST_FIELD.id to InterntFelt("asdf")))
                .toExternalDTO(
                    EMPTY_ADMIN_DATA.copy(internalFields = INTERNAL_FIELDS, codeLists = CODE_LISTS),
                    "http://localhost:5555",
                )
    ...
    fun `Map user list internal fields`() {
        ...
        val result =
            EMPTY_INTERNAL_CONCEPT
                .copy(interneFelt = mapOf(USER_LIST_FIELD.id to InterntFelt(adminUser.id)))
                .toExternalDTO(
                    EMPTY_ADMIN_DATA.copy(
                        internalFields = INTERNAL_FIELDS,
                        users = mapOf("${adminUser.catalogId}-${adminUser.id}" to adminUser),
                    ),
                    "http://localhost:5555",
                )
    ...
    fun `Map seeAlso`() {
        ...
        val result =
            EMPTY_INTERNAL_CONCEPT
                .copy(seOgså = listOf("https://concept-0.no"))
                .toExternalDTO(EMPTY_ADMIN_DATA, "http://localhost:5555")
    ...
    fun `Map replacedBy`() {
        ...
        val result =
            EMPTY_INTERNAL_CONCEPT
                .copy(erstattesAv = listOf("https://concept-1.no"))
                .toExternalDTO(EMPTY_ADMIN_DATA, "http://localhost:5555")
            ...
            EMPTY_CONCEPT.copy(
                ...
                listOf(
                    ConceptRelation(
                        ...
                        description = LocalizedStrings(nb = null, nn = null, en = null),
                        relatedConcept = "https://concept-0.no",
                    ),
                    ConceptRelation(
                        relationType = "ASSOCIATIVE",
                        description = LocalizedStrings(nb = "beskrivelse", nn = null, en = null),
        ...
        val expected =
            EMPTY_CONCEPT.copy(
                ...
                listOf(
                    ConceptRelation(
                        relationType = null,
                        description = LocalizedStrings(null, null, null),
                        relatedConcept = "https://concept-0.no",
                    ),
                    ConceptRelation(
    ...
    fun `Map attachedTag`() {
        ...
        val result =
            EMPTY_INTERNAL_CONCEPT
                .copy(merkelapp = listOf("merkelapp", "tag"))
                .toExternalDTO(EMPTY_ADMIN_DATA, "http://localhost:5555")
