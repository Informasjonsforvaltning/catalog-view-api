package no.digdir.catalogview.service

import no.digdir.catalogview.config.ApplicationProperties
import no.digdir.catalogview.model.AdminCode
import no.digdir.catalogview.model.AdminUser
import no.digdir.catalogview.model.BegrepsRelasjon
import no.digdir.catalogview.model.BooleanField
import no.digdir.catalogview.model.CatalogAdminData
import no.digdir.catalogview.model.Code
import no.digdir.catalogview.model.CodeField
import no.digdir.catalogview.model.CodeList
import no.digdir.catalogview.model.Concept
import no.digdir.catalogview.model.ConceptRelation
import no.digdir.catalogview.model.ContactPoint
import no.digdir.catalogview.model.Definisjon
import no.digdir.catalogview.model.Definition
import no.digdir.catalogview.model.EditableFields
import no.digdir.catalogview.model.Field
import no.digdir.catalogview.model.FieldInterface
import no.digdir.catalogview.model.ForholdTilKildeEnum
import no.digdir.catalogview.model.InternalConcept
import no.digdir.catalogview.model.ListOfLocalizedStrings
import no.digdir.catalogview.model.LocalizedStrings
import no.digdir.catalogview.model.LongTextField
import no.digdir.catalogview.model.ShortTextField
import no.digdir.catalogview.model.SourceDescription
import no.digdir.catalogview.model.URITekst
import no.digdir.catalogview.model.URIText
import no.digdir.catalogview.model.User
import no.digdir.catalogview.model.UserField
...
class ConceptsService(
    ...
    private fun getAllConcepts(): List<InternalConcept> = conceptCatalogDB.query("SELECT data FROM concepts") { rs, _ ->
        objectMapper.readValue(rs.getString("data"), InternalConcept::class.java)
    }

    private fun getAllAdminData(): CatalogAdminData = CatalogAdminData(
    ...
    private fun getCodeListsFromAdminService(): Map<String, CodeList> = adminServiceDB
        .query("SELECT id, catalog_id, codes FROM code_lists") { rs, _ ->
                ...
                rs
                    ...
                    ?.let {
                        objectMapper.readValue(
                            it,
                            objectMapper.typeFactory.constructCollectionType(List::class.java, AdminCode::class.java),
                        )
                    }
                    ?: emptyList()
            CodeList(
    ...
    private fun getInternalFieldsFromAdminService(): Map<String, Field> = adminServiceDB
        .query("SELECT id, catalog_id, label, description, type, code_list_id FROM internal_fields") { rs, _ ->
            Field(
                id = rs.getString("id"),
                catalogId = rs.getString("catalog_id"),
                label =
    ...
    private fun getDomainCodeListIdFromAdminService(): Map<String, String?> = adminServiceDB
        .query("SELECT catalog_id, domain_code_list_id FROM editable_fields") { rs, _ ->
            EditableFields(
                catalogId = rs.getString("catalog_id"),
    ...
    private fun getUsersFromAdminService(): Map<String, AdminUser> = adminServiceDB
        .query("SELECT id, catalog_id, name, email, telephone_number FROM catalog_users") { rs, _ ->
            AdminUser(
                id = rs.getString("id"),
                catalogId = rs.getString("catalog_id"),
...
private fun AdminUser.toDTO(): User = User(
    name = name,
    email = email,
)
...
fun InternalConcept.toExternalDTO(adminData: CatalogAdminData, baseURI: String): Concept {
    ...
    return Concept(
        ...
        domainCodes =
        fagområdeKoder
            ?.mapNotNull { adminData.getDomainCode(it, ansvarligVirksomhet.id) }
            ?.ifEmpty { null },
        startDate = gyldigFom,
        endDate = gyldigTom,
        created = opprettet,
...
private fun Definisjon.toDefinition(): Definition = Definition(
...
private fun ForholdTilKildeEnum.toURI(): String = when (this) {
...
private fun URITekst.toURIText(): URIText = URIText(uri = uri, text = tekst)

private fun CatalogAdminData.getDomainCode(id: String?, catalogId: String): Code? {
    val codeListId = domainCodeList[catalogId]

    return if (id == null || codeListId == null) {
        null
    } else {
        codeLists["$catalogId-$codeListId"]
            ?.codes
            ?.find { it.id == id }
...
private fun String.safeToInt(): Int? = try {
    toInt()
} catch (_: Exception) {
    null
}
...
private fun Field.toBooleanField(value: String): BooleanField? = value
...
private fun Field.toShortTextField(value: String): ShortTextField = ShortTextField(
    id = id,
    label = label,
    value = value,
)

private fun Field.toLongTextField(value: String): LongTextField = LongTextField(
    id = id,
    label = label,
    value = value,
)

private fun Field.toCodeField(value: String, catalogId: String, codeLists: Map<String, CodeList>): CodeField? = if (codeListId == null) {
    null
} else {
    codeLists["$catalogId-$codeListId"]
        ?.codes
...
private fun String?.toRelationType(subType: String?): RelationType? = when {
...
private fun BegrepsRelasjon.toAssociativeRelation(): ConceptRelation = ConceptRelation(
...
private fun BegrepsRelasjon.toGenericOrPartitiveRelation(relationType: RelationType?): ConceptRelation = ConceptRelation(
...
private fun BegrepsRelasjon.toConceptRelation(): ConceptRelation {
    ...
    return if (relationType == RelationType.ASSOCIATIVE) {
        toAssociativeRelation()
    } else {
        toGenericOrPartitiveRelation(relationType)
    }
}

private fun BegrepsRelasjon.toInternalConceptRelation(conceptBaseURI: String): ConceptRelation = toConceptRelation()
    .copy(relatedConcept = relatertBegrep?.let { conceptURI(it, conceptBaseURI) })

private fun InternalConcept.collectionURI(baseURI: String) = "$baseURI/collections/${ansvarligVirksomhet.id}"

private fun conceptURI(conceptID: String, collectionURI: String) = "$collectionURI/concepts/$conceptID"
