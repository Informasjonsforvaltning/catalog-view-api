    ...
    private fun getCodeListsFromAdminService(): Map<String, CodeList> =
        adminServiceDB.query("SELECT id, catalog_id, codes FROM code_lists") { rs, _ ->
            val codes: List<AdminCode> = rs.getString("codes")
                ?.let {
                    objectMapper.readValue(it, objectMapper.typeFactory.constructCollectionType(List::class.java, AdminCode::class.java))
                }
                ?: emptyList()
            CodeList(
                id = rs.getString("id"),
...
private fun CatalogAdminData.getDomainCode(id: String?, catalogId: String): Code? {
    ...
    return if (id == null || codeListId == null) {
        ...
        codeLists["$catalogId-$codeListId"]?.codes
            ?.find { it.id == id }
...
private fun Field.toBooleanField(value: String): BooleanField? = value.toBooleanStrictOrNull()
    ?.let {
        BooleanField(
            id = id,
            label = label,
...
private fun Field.toCodeField(value: String, catalogId: String, codeLists: Map<String, CodeList>): CodeField? = if (codeListId == null) {
    ...
    codeLists["$catalogId-$codeListId"]?.codes?.find { it.id == value }
        ?.let {
            CodeField(
                id = id,
                label = label,
